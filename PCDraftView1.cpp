...
IMPLEMENT_DYNCREATE(CPCDraftView, CView)

BEGIN_MESSAGE_MAP(CPCDraftView, CView)
...
void CPCDraftView::OnDraw(CDC* pDC)
{
   ...
   if(pDoc->m_bComplete) // completely opened
   {
      ...
      if(m_bEditActive)
      {
         CDraftContext theContext(pDC,&m_Mapper,pDoc);
         TextParams tp;
...
//	CView::OnInitialUpdate();
   ...
   {
      m_wndPosition.SetWindowText(pDoc->GetTitle());
   }
...
void CPCDraftView::GetClientSize(CSize& size)
{
   ...
   {
      size.cx-=kLRulerWid+2;
      size.cy-=kTRulerWid+2;
   }
}
...
void CPCDraftView::GetViewRect(CRect& rectView)
{
   ...
   if (GetDocument()->m_bShowRulers)
   {
      ...
      rectView.left += kLRulerWid + 1;

      rectView.top += kTRulerWid + 1;
...
void CPCDraftView::ScrollWindow(int dx,int dy)
{
   ...
   //clientRect.SetRect(0,0,clientSize.cx,clientSize.cy);
   ...
   if(GetDocument()->m_bShowRulers)
   {
      ...
      if (dy)
      {
         ...
         regionLeftRuler.CreateRectRgn(0, 0, kLRulerWid + 1, rectView.bottom);
      ...
      if (dx)
      {
         ...
         regionTopRuler.CreateRectRgn(0, 0, rectView.right, kTRulerWid + 1);
      ...
      if (dx && dy)
      {
         ...
         regionRulers.CreateRectRgn(0, 0, 0, 0);

         regionRulers.CombineRgn(&regionLeftRuler, &regionTopRuler, RGN_OR);
      ...
      else if (dy)
      {
         ...
         regionRulers.CopyRgn(&regionLeftRuler);
      ...
      else if (dx)
      {
         ...
         regionRulers.CopyRgn(&regionTopRuler);

      }

      CRect rectScroll(rectView);

      //CRect rectClip(rectScroll);

      CRect rectUpdate(rectScroll);

      if (dy < 0)
      {

         rectUpdate.top = rectUpdate.bottom + dy - 1;
      ...
      else if (dy > 0)
      {
         ...
         rectUpdate.bottom = rectUpdate.top + dy + 1;
      ...
      if (dx < 0)
      {
         ...
         rectUpdate.left = rectUpdate.right + dx - 1;
      ...
      else if(dx > 0)
      {
         ...
         rectUpdate.right = rectUpdate.left + dx + 1;
      ...
      regionUpdate.CreateRectRgnIndirect(rectUpdate);

      CRgn regionInvalidate;

      regionInvalidate.CreateRectRgn(0, 0, 0, 0);

      regionInvalidate.CombineRgn(&regionRulers, &regionUpdate, RGN_OR);

      ScrollWindowEx(dx, dy, rectClient, rectView, &regionInvalidate, NULL, SW_INVALIDATE);
      ...
      //   clipRect.SetRect(kLRulerWid+2,0,clientRect.right,kTRulerWid+1);
      //   CView::ScrollWindow(dx,0,clipRect,clipRect);
      //   clipRect.SetRect(0,kTRulerWid+2,kLRulerWid+1,clientRect.bottom);
      //   CView::ScrollWindow(0,dy,clipRect,clipRect);
      //}
      //else if(dx)
      //{
      //   clipRect.SetRect(kLRulerWid+2,0,clientRect.right,clientRect.bottom);
      //   CView::ScrollWindow(dx,0,clipRect,clipRect);
      //}
      //else if(dy)
      //{
      //   clipRect.SetRect(0,kTRulerWid+2,clientRect.right,clientRect.bottom);
      //   CView::ScrollWindow(0,dy,clipRect,clipRect);
      //}
   }
   else
      CView::ScrollWindow(dx,dy,rectView,rectView);
}
...
void CPCDraftView::DrawRulers(CDC * pDC, CRgn* pClip)
{
   ...
   if(NULLREGION!=pDC->SelectClipRgn(&theClipRgn,RGN_AND))
   {
      ...
      for(int i=start; i<ptOrg.x+rc.right; i+=nRes)
      {
         ...
         {
            pDC->MoveTo(kLRulerWid+2+j,kTRulerWid);
            pDC->LineTo(kLRulerWid+2+j,kTRulerWid-nMarkLen[k]-1);
            k=(k+1)%nNumMarks;
         }
         posN=Abs(n);
         if(GetDocument()->m_bStandardRulers)
   ...
   if(NULLREGION!=pDC->SelectClipRgn(&theClipRgn,RGN_AND))
   {
      ...
      for(int i=start; i<ptOrg.y+rc.bottom; i+=nRes)
      {
         ...
         for(int j=i; j<end; j+=nMinTicks)	/* draw ticks for one inch */
         {
            pDC->MoveTo(kLRulerWid,kTRulerWid+j+2);
            pDC->LineTo(kLRulerWid-nMarkLen[k]-1,kTRulerWid+j+2);
            k=(k+1)%nNumMarks;
         }
         posN=Abs(n);
         if(GetDocument()->m_bStandardRulers)
         ...
         if(len>3)				/* 2nd char below tick */
            pDC->TextOut(0,0,c++,1);
         if(len>1)
            pDC->MoveTo(kVOffsetH,kTRulerWid+2+i+kVOffset1);
         while(*c)
            pDC->TextOut(0,0,c++,1);
         n+=nMarkStep;
      }
...
void CPCDraftView::ConstructGridLines()
{
   ...
   int res=GetDocument()->NoZoomToZoom(GetDocument()->m_nDisplayRes);
...
void CPCDraftView::DrawGridLines(CDC* pDC, CPoint ptOrg)
{
   CSize clientSize;
   GetClientSize(clientSize);
   int nRes=GetDocument()->NoZoomToZoom(GetDocument()->m_nDisplayRes);
   ...
   /**********************************************/
   {
      ...
      {
         pDC->BitBlt(ptOffset.x+i,ptOffset.y,1,m_totalDev.cy+1,&theMemDC,0,nRes-vOrgOff%nRes,SRCCOPY);
      }
   }
   /************************************************/
...
void CPCDraftView::OnLButtonDown(UINT nFlags, CPoint point)
{
   ...
   if (GetDocument()->m_bShowGuideLines && GetAsyncKeyState(VK_COMMAND) < 0 && GetAsyncKeyState(VK_MENU) < 0)
   {
      ...
      else
      {
         ...
         else
         {
            ...
            if(pDoc->m_nZoomFactor<pDoc->m_nMaxZoom)
            {
               ...
               if(theZoomSketcher.DoSketch(this,pt,&rcClient,CSketcher::NoSnap|CSketcher::NoScroll))
               {
                  SaveUndo(0);
                  if(theZoomSketcher.m_bArea)
                  {
   ...
   while(1)
   {
      ...
      if(PeekMessage(&msg,NULL,0,0,PM_REMOVE))
      {
         ...
         case WM_MOUSEMOVE:
            ...
            if(pt.x>=docSize.cy) pt.y=docSize.cy-1;
...
void CPCDraftView::OnToolsRange(UINT nID)
{
   ...
   if(nID!=ID_TOOL_TEXT) DeactivateTextShape();
   ...
   while(1)
   {
      ...
      if(PeekMessage(&msg,NULL,0,0,PM_REMOVE))
      {
         ...
         case WM_TIMER:
         {
            ...
            SnapSelectionToGuide(ptNew, ptOrig, nullptr);
...
{
   if(m_bEditActive) m_InplaceEdit.DoPaste();
   else DoEditPaste();
}
...
void CPCDraftView::DoEditPaste(bool bUnscaled, CPoint* ptAt)
{
   if(OpenClipboard())
   {
      SaveUndo(IDS_PASTE);
      ...
      if(hClip)
      {
         CreateShapesFromHandle(hClip,bUnscaled,ptAt?ptAt:&m_ptPaste);
         GetDocument()->CheckDocSize(true);
      }
      else
      {
         ...
         if(hClip)
         {
            ...
            }
            {
               ...
               dc.CalcBoundRect(st,GetDocument()->m_TextParams,rc);
            }
            LongRect lrc=m_Mapper.DPtoLP(rc);
            lrc.Inset(-3,-3);
            lrc.Offset(lptOffset);
...
void CPCDraftView::OnEditDuplucate()
{
   ...
   SelectNone();
   CPoint ptDupDelta;
   if(GetAsyncKeyState(VK_SHIFT)<0) ptDupDelta= CPoint(0,0); // duplicate on top
...
void CPCDraftView::OnUpdateToolAttribLineStyle(CCmdUI* pCmdUI)
{
   ...
   CBitmap bmp;
   CClientDC cdc(NULL);
   bmp.CreateCompatibleBitmap(&cdc,32,1);
   {
      CGDIBitmapDC dc(&cdc,&bmp);
      dc.MoveTo(0,0);

      CGDIBackColor bc(&dc,RGB(0,0,0));
      CGDITextColor tc(&dc,RGB(255,255,255));

      CDrawPrim dp;
      dp.setDashTable((const CDrawPrim::DashTable*)pDoc->m_DashStyles);
      CDraftContext pdc(&dc,&m_Mapper,pDoc);
      pdc.SetPen(pm);
      dp.dashLine(&pdc,pointd(0,0), pointd(32,0),true);
   }
...
//   {
      ...
      //b.CreateCompatibleBitmap(&dcPaint,r.Width(),r.Height());

      //HGDIOBJ h = dcMem.SelectObject(b);

      //CPoint ptOrg=GetDeviceScrollPosition();
      //dcMem.SetWindowOrg(ptOrg);

      CDC & dc = dcPaint;

  //    OnEraseBkgnd(pgraphics);
      OnEraseBkgnd(&dc);
      OnPrepareDC(&dc, NULL);
      OnDraw(&dc);
   ...
   //   CPoint ptOrg=GetDeviceScrollPosition();
   //   dcPaint.SetViewportOrg(-ptOrg);


   //   dcPaint.BitBlt(0,0,r.Width(),r.Height(),&dc,0,0,SRCCOPY);
