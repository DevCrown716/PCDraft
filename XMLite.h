...
typedef struct _tagXMLNode
{
   ...
   LPTSTR	LoadDOCTYPE(LPCTSTR pszXml,LPPARSEINFO pi = &piDefault);
   LPTSTR	LoadComment(LPCTSTR pszXml,LPPARSEINFO pi = &piDefault);
	LPTSTR	LoadCDATA( LPCTSTR pszXml, LPPARSEINFO pi = &piDefault ); 

   LPXNode get_pinfo_node(CString strKeyName);
   bool get_pinfo_string(CString & strValue, CString strKeyName);
   bool get_pinfo_real(double & dReal, CString strKeyName);
   bool get_pinfo_integer(int & iInteger,CString strKeyName);
   bool get_child_numbera(CArray < double > & numbera);
   ...
   int	IndexByValue(LPCTSTR name,LPCTSTR value);
   XNode * GetChild(LPCTSTR name,LPCTSTR value);
