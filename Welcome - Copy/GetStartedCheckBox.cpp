...
void GetStartedCheckBox::OnDraw(CDC * pdc, Gdiplus::Graphics * pgraphics)
{
   ...
   if (m_bChecked)
   {
      ...
      pgraphics->DrawImage(m_pbitmapChecked, 0, 0, m_pbitmapChecked->GetWidth() * size_scaler(), m_pbitmapChecked->GetHeight() * size_scaler());

   }
   else
   {

      pgraphics->DrawImage(m_pbitmapUnchecked, 0, 0, m_pbitmapChecked->GetWidth() * size_scaler(), m_pbitmapChecked->GetHeight() * size_scaler());
   ...
   Gdiplus::RectF rectF(m_pbitmapChecked->GetWidth() + 6, 0, rectClient.Width(), rectClient.Height());
