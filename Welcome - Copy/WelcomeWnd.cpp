...
void WelcomeWnd::OnSize(UINT nType, int cx, int cy)
{
   ...
   // Body
   {
      ...
      int y = (40 + 128 + 92 + 80) * size_scaler();

      int iEditHeight = 70 * size_scaler();

      int iEditSpacing = 20 * size_scaler();

      int x = 150 * size_scaler();
      ...
      m_rectMessage.Height = iEditHeight * 3;
   ...
   // Footer
   {
      ...
      int wButton = 120 * size_scaler();

      int HSpacing = 30 * size_scaler();

      int x = (rectClient.Width() - (wButton * 2) - HSpacing) / 2;

      int hFooter = 65 * size_scaler();

      int y = rectClient.Height() - hFooter;

      m_buttonCancel.SetWindowPos(nullptr, x, y, wButton, 40 * size_scaler(), SWP_SHOWWINDOW | SWP_NOZORDER);

      m_buttonOk.SetWindowPos(nullptr, x + wButton + HSpacing, y, wButton, 40 * size_scaler(), SWP_SHOWWINDOW | SWP_NOZORDER);
...
void WelcomeWnd::OnDraw(CDC * pdc, Gdiplus::Graphics * pgraphics)
{
   ...
   // Top Center Icon
   {
      ...
      rectIcon.Width = 128 * size_scaler();
      rectIcon.Height = 128 * size_scaler();
      rectIcon.Y = 40 * size_scaler();
   ...
   // Top Center Title
   {
      ...
      Gdiplus::RectF        rectF(0, (40 + 128)  * size_scaler(), rectClient.Width(), rectClient.Height() / 2);
   ...
   // Message
   {
      ...
      pgraphics->DrawString(wstr, wstr.GetLength(), m_pfontMessage, m_rectMessage, &format, &brush);
