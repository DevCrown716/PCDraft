   ...
   for (int i = 0; i < iSize; i++)
   {
      ...
      if (MatchDocument(strPath, strFilter))
      {
            ...
            && file.operator HANDLE() != nullptr)
         {
            ...
            if (iHours < 24 && time.GetDay() == timeNow.GetDay())
            {

               strGroup.LoadStringA(IDS_RECENTWND_TODAY);

            }
            else if (iHours < 48 &&
               (time.GetDay() + 1 == timeNow.GetDay()
                  || (timeNow.GetDay() == 1 && time.GetDay() >= 28)))
            {

               strGroup.LoadStringA(IDS_RECENTWND_YESTERDAY);
               ...
               && timeNow.GetMonth() == time.GetMonth())
            {
               ...
               strGroup.LoadStringA(IDS_RECENTWND_THIS_MONTH);

            }
            else if (timeNow.GetYear() == time.GetYear()
               && (timeNow.GetMonth() == time.GetMonth() + 1
                  || (timeNow.GetMonth() == 1 && time.GetMonth() == 12)))
            {

               strGroup.LoadStringA(IDS_RECENTWND_LAST_MONTH);

            }
            else
            {

               strGroup.LoadStringA(IDS_RECENTWND_EARLIER);
...
void TemplateRecentWnd::OnLButtonUp(UINT nFlags, CPoint point)
{
   ...
   if(hit_test(iGroup, iItem, point))
   {
      ...
      if (iGroup == m_iSelGroup && iItem == m_iSelItem)
      {
...
void TemplateRecentWnd::layout()
{
   ...
   for (int i = 0; i < m_groupa.GetCount(); i++)
   {
      ...
      group.m_title.m_rect.right = cx - group.m_title.m_rect.left -32;
      ...
      for (int j = 0; j < group.m_itema.GetCount(); j++)
      {
         ...
         item.m_rect.left = 20;
         item.m_rect.right = cx - item.m_rect.left - 32;
         item.m_rect.top = y;
         y += iButtonHeight;
   ...
   if (hit_test(iHoverGroup, iHoverItem, point))
   {
      ...
      if (iHoverItem != m_iHoverItem || iHoverGroup != m_iHoverGroup)
      {
            ...
            && iHoverItemOld >= 0 && iHoverItemOld < m_groupa[iHoverGroupOld].m_itema.GetCount())
         {
            ...
            auto rect = m_groupa[iHoverGroupOld].m_itema[iHoverItemOld].m_rect - m_scrollHelper->GetScrollPos();

            rect.InflateRect(10 * size_scaler(), 10.0 * size_scaler());
            ...
            && iHoverItem >= 0 && iHoverItem < m_groupa[iHoverGroup].m_itema.GetCount())
         {
            ...
            auto rect = m_groupa[iHoverGroup].m_itema[iHoverItem].m_rect - m_scrollHelper->GetScrollPos();
