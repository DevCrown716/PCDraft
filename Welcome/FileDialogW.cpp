...
UINT _afxMsgLBSELCHANGE2 = 0;
UINT _afxMsgSHAREVI2= 0;
UINT _afxMsgFILEOK2 = 0;
UINT _afxMsgCOLOROK2 = 0;
UINT _afxMsgHELP2 = 0;
UINT _afxMsgSETRGB2 = 0;
#define UNICODE
...
UINT_PTR CALLBACK _AfxCommDlgProc2(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam);

////////////////////////////////////////////////////////////////////////////
// FileOpen/FileSaveAs common dialog helper

CFileDialogW::CFileDialogW(BOOL bOpenFileDialog,
   LPCWSTR lpszDefExt, LPCWSTR lpszFileName, DWORD dwFlags,
   LPCWSTR lpszFilter, CWnd* pParentWnd, DWORD dwSize, BOOL bVistaStyle)
...
#pragma warning( default : 4996 )
   ...
   ASSERT(dwSize >= sizeof(OPENFILENAMEW));
   // allocate memory for OPENFILENAME struct based on size passed in
   m_pOFN = static_cast<LPOPENFILENAMEW>(malloc(dwSize));
   ASSERT(m_pOFN != NULL);
   if (m_pOFN == NULL)
   ...
   m_ofn.lpstrFileTitle = (LPWSTR)m_szFileTitle;
   ...
   m_ofn.lpfnHook = (COMMDLGPROC)_AfxCommDlgProc2;

   // setup initial file name
   if (lpszFileName != NULL)
      Checked::wcsncpy_s(m_szFileName, _countof(m_szFileName), lpszFileName, _TRUNCATE);

   // Translate filter into commdlg format (lots of \0)
   if (lpszFilter != NULL)
   {
      m_strFilter = lpszFilter;
      LPWSTR pch = m_strFilter.GetBuffer(0); // modify the buffer in place
...
const OPENFILENAMEW& CFileDialogW::GetOFN() const
{
   return *m_pOFN;
}

OPENFILENAMEW& CFileDialogW::GetOFN()
{
   return *m_pOFN;
}

void CFileDialogW::UpdateOFNFromShellDialog()
{
   ...
   if (m_bVistaStyle == TRUE)
   {
      ...
      if (SUCCEEDED(hr))
      {
         ...
         if (SUCCEEDED(hr))
         {
            ...
            m_ofn.lpstrFileTitle[m_ofn.nMaxFileTitle - 1] = L'\0');
            ::WideCharToMultiByte(CP_ACP, 0, wcPathName, -1, m_ofn.lpstrFile,
               m_ofn.nMaxFile - 1, NULL, NULL);
            m_ofn.lpstrFile[m_ofn.nMaxFile - 2] = L'\0');
#endif
            m_ofn.lpstrFile[wcslen(m_ofn.lpstrFile) + 1] = L'\0';
         ...
         if (SUCCEEDED(hr))
         {
            ...
            if (SUCCEEDED(hr))
            {
               ...
               if (SUCCEEDED(hr))
               {
                  ...
                  {
                     CStringW strTmp;
                     LPWSTR pszFileName = m_ofn.lpstrFile;
                     LPWSTR wcPathName = NULL;
                     ...
                     {
                        m_ofn.lpstrFile[m_ofn.nMaxFile - 2] = L'\0';
                        m_ofn.lpstrFile[m_ofn.nMaxFile - 1] = L'\0';
                     }
                  }
      ...
      CStringW strPathName = GetPathName();

      CStringW strFileName;
      LPWSTR pszFilename = ::PathFindFileNameW(strPathName);
      if (pszFilename != NULL)
      {
      ...
      LPWSTR pszExtension = ::PathFindExtensionW(strPathName);
      if (pszExtension != NULL && *pszExtension == L'.')
      {
...
void CFileDialogW::ApplyOFNToShellDialog()
{
   ASSERT(m_bVistaStyle == TRUE);
   if (m_bVistaStyle == TRUE)
   {
      ...
      {
         UINT nFilterCount = 0;
         LPCWSTR lpstrFilter = m_ofn.lpstrFilter;
         while (lpstrFilter[0])
         {
         ...
         if (nFilterCount > 0)
         {
            ...
            while (nFilterIndex < nFilterCount)
            {
               ...
               filterSize = wcslen(lpstrFilter) + 1;
...
IFileOpenDialog* CFileDialogW::GetIFileOpenDialog()
{
   ASSERT(m_bVistaStyle == TRUE);
...
IFileSaveDialog* CFileDialogW::GetIFileSaveDialog()
{
   ASSERT(m_bVistaStyle == TRUE);
...
IFileDialogCustomize* CFileDialogW::GetIFileDialogCustomize()
{
   ASSERT(m_bVistaStyle == TRUE);
...
void CFileDialogW::AddPlace(LPCWSTR lpszFolder, FDAP fdap)
{
   if (!m_bVistaStyle)
   {
      return;
...
void CFileDialogW::AddPlace(IShellItem* psi, FDAP fdap)
{
   if (!m_bVistaStyle)
   {
      return;
...
BEGIN_INTERFACE_MAP(CFileDialogW, CCmdTarget)
   INTERFACE_PART(CFileDialogW, IID_IFileDialogEvents, FileDialogEvents)
   INTERFACE_PART(CFileDialogW, IID_IFileDialogControlEvents, FileDialogControlEvents)
END_INTERFACE_MAP()

STDMETHODIMP_(ULONG) CFileDialogW::XFileDialogEvents::AddRef()
{
   METHOD_PROLOGUE(CFileDialogW, FileDialogEvents)
      return pThis->ExternalAddRef();
}

STDMETHODIMP_(ULONG) CFileDialogW::XFileDialogEvents::Release()
{
   METHOD_PROLOGUE(CFileDialogW, FileDialogEvents)
      return pThis->ExternalRelease();
}

STDMETHODIMP CFileDialogW::XFileDialogEvents::QueryInterface(
...
STDMETHODIMP CFileDialogW::XFileDialogEvents::OnFileOk(IFileDialog *)
{
   METHOD_PROLOGUE(CFileDialogW, FileDialogEvents)
...
STDMETHODIMP CFileDialogW::XFileDialogEvents::OnFolderChange(IFileDialog *)
{
   METHOD_PROLOGUE(CFileDialogW, FileDialogEvents)
...
STDMETHODIMP CFileDialogW::XFileDialogEvents::OnFolderChanging(IFileDialog *, IShellItem *)
{
   METHOD_PROLOGUE(CFileDialogW, FileDialogEvents)
      return S_OK;
}

STDMETHODIMP CFileDialogW::XFileDialogEvents::OnHelp(IFileDialog *)
{
   METHOD_PROLOGUE(CFileDialogW, FileDialogEvents)
      return S_OK;
}

STDMETHODIMP CFileDialogW::XFileDialogEvents::OnSelectionChange(IFileDialog *)
{
   METHOD_PROLOGUE(CFileDialogW, FileDialogEvents)
...
STDMETHODIMP CFileDialogW::XFileDialogEvents::OnTypeChange(IFileDialog *)
{
   METHOD_PROLOGUE(CFileDialogW, FileDialogEvents)
      UINT uIdx = 0;
...
STDMETHODIMP CFileDialogW::XFileDialogEvents::OnOverwrite(IFileDialog *, IShellItem *, FDE_OVERWRITE_RESPONSE *)
{
   METHOD_PROLOGUE(CFileDialogW, FileDialogEvents)
      return S_OK;
}

STDMETHODIMP CFileDialogW::XFileDialogEvents::OnShareViolation(
   IFileDialog *,
   IShellItem *psi,
   FDE_SHAREVIOLATION_RESPONSE *pResponse)
{
   ...
   ENSURE(SUCCEEDED(hr));
   CStringW strPathName(wcPathName);
   CoTaskMemFree(wcPathName);
...
STDMETHODIMP_(ULONG) CFileDialogW::XFileDialogControlEvents::AddRef()
{
   METHOD_PROLOGUE(CFileDialogW, FileDialogControlEvents)
      return pThis->ExternalAddRef();
}

STDMETHODIMP_(ULONG) CFileDialogW::XFileDialogControlEvents::Release()
{
   METHOD_PROLOGUE(CFileDialogW, FileDialogControlEvents)
      return pThis->ExternalRelease();
}

STDMETHODIMP CFileDialogW::XFileDialogControlEvents::QueryInterface(
   REFIID iid, void FAR* FAR* ppvObj)
{
   METHOD_PROLOGUE(CFileDialogW, FileDialogControlEvents)
      ENSURE(ppvObj != NULL);
...
STDMETHODIMP CFileDialogW::XFileDialogControlEvents::OnItemSelected(IFileDialogCustomize *, DWORD dwIDCtl, DWORD dwIDItem)
{
   METHOD_PROLOGUE(CFileDialogW, FileDialogControlEvents)
...
STDMETHODIMP CFileDialogW::XFileDialogControlEvents::OnButtonClicked(IFileDialogCustomize *, DWORD dwIDCtl)
{
   METHOD_PROLOGUE(CFileDialogW, FileDialogControlEvents)
...
STDMETHODIMP CFileDialogW::XFileDialogControlEvents::OnCheckButtonToggled(IFileDialogCustomize *, DWORD dwIDCtl, BOOL bChecked)
{
   METHOD_PROLOGUE(CFileDialogW, FileDialogControlEvents)
...
STDMETHODIMP CFileDialogW::XFileDialogControlEvents::OnControlActivating(IFileDialogCustomize *, DWORD dwIDCtl)
{
   METHOD_PROLOGUE(CFileDialogW, FileDialogControlEvents)
...
INT_PTR CFileDialogW::DoModal()
{
   ...
   DWORD nOffset = static_cast<DWORD>(wcslen(m_ofn.lpstrFile)) + 1;
   ASSERT(nOffset <= m_ofn.nMaxFile);
   memset(m_ofn.lpstrFile + nOffset, 0, (m_ofn.nMaxFile - nOffset) * sizeof(WCHAR));
...
IShellItem *CFileDialogW::GetResult()
{
...
IShellItemArray *CFileDialogW::GetResults()
{
...
CStringW CFileDialogW::GetPathName() const
{
   if (m_bVistaStyle == TRUE)
   {
      if (m_hWnd != NULL)
...
CStringW CFileDialogW::GetFileName() const
{
   if (m_bVistaStyle == TRUE)
   {
      if (m_hWnd != NULL)
...
CStringW CFileDialogW::GetFileExt() const
{
   if (m_bVistaStyle == TRUE)
   {
      CStringW strResult;
      ...
      strResult.ReleaseBuffer();
      LPWSTR pszExtension = ::PathFindExtensionW(strResult);
      if (pszExtension != NULL && *pszExtension == L'.')
      {
   ...
   else if ((m_ofn.Flags & OFN_EXPLORER) && m_hWnd != NULL)
   {
      ASSERT(::IsWindow(m_hWnd));
      CStringW strResult;
      LPWSTR pszResult = strResult.GetBuffer(MAX_PATH);
...
CStringW CFileDialogW::GetFileTitle() const
{
   CStringW strResult = GetFileName();
   LPWSTR pszBuffer = strResult.GetBuffer();
   ::PathRemoveExtensionW(pszBuffer);
   strResult.ReleaseBuffer();
   return strResult;
}

CStringW CFileDialogW::GetNextPathName(POSITION& pos) const
{
   ...
   if (*strDrive || *strDir)
   {
      Checked::wcscpy_s(strPath, _countof(strPath), strFileName);
   }
   else
   {
      ...
      _wsplitpath_s(strBasePath, strDrive, _MAX_DRIVE, strDir, _MAX_DIR, NULL, 0, NULL, 0);
...
void CFileDialogW::SetTemplate(LPCWSTR lpWin3ID, LPCWSTR lpWin4ID)
{
   if (m_bVistaStyle == TRUE)
   {
...
CStringW CFileDialogW::GetFolderPath() const
{
   CStringW strResult;
   if (m_bVistaStyle == TRUE)
   {
...
void CFileDialogW::SetControlText(int nID, LPCWSTR lpsz)
{
   ASSERT(::IsWindow(m_hWnd));
...
void CFileDialogW::HideControl(int nID)
{
   ASSERT(::IsWindow(m_hWnd));
   if (m_bVistaStyle == TRUE)
   {
...
void CFileDialogW::SetDefExt(LPCWSTR lpsz)
{
   ASSERT(::IsWindow(m_hWnd));
   if (m_bVistaStyle == TRUE)
   {
...
UINT CFileDialogW::OnShareViolation(LPCWSTR)
{
   ASSERT_VALID(this);
...
BOOL CFileDialogW::OnFileNameOK()
{
   ASSERT_VALID(this);
...
void CFileDialogW::OnLBSelChangedNotify(UINT, UINT, UINT)
{
   ASSERT_VALID(this);
...
void CFileDialogW::OnFileNameChange()
{
   ASSERT_VALID(this);
...
void CFileDialogW::OnFolderChange()
{
   ASSERT_VALID(this);
...
void CFileDialogW::OnTypeChange()
{
   ASSERT_VALID(this);
...
BOOL CFileDialogW::SetProperties(LPCWSTR lpszPropList)
{
   ASSERT(m_bVistaStyle == TRUE);
...
HRESULT CFileDialogW::EnableOpenDropDown(DWORD dwIDCtl)
{
   if (!m_bVistaStyle)
   {
      return E_NOTIMPL;
...
HRESULT CFileDialogW::AddMenu(DWORD dwIDCtl, const CStringW& strLabel)
{
   if (!m_bVistaStyle)
   {
      return E_NOTIMPL;
...
HRESULT CFileDialogW::AddPushButton(DWORD dwIDCtl, const CStringW& strLabel)
{
   if (!m_bVistaStyle)
   {
      return E_NOTIMPL;
...
HRESULT CFileDialogW::AddComboBox(DWORD dwIDCtl)
{
   if (!m_bVistaStyle)
   {
      return E_NOTIMPL;
...
HRESULT CFileDialogW::AddRadioButtonList(DWORD dwIDCtl)
{
   if (!m_bVistaStyle)
   {
      return E_NOTIMPL;
...
HRESULT CFileDialogW::AddCheckButton(DWORD dwIDCtl, const CStringW& strLabel, BOOL bChecked)
{
   if (!m_bVistaStyle)
   {
      return E_NOTIMPL;
...
HRESULT CFileDialogW::AddEditBox(DWORD dwIDCtl, const CStringW& strText)
{
   if (!m_bVistaStyle)
   {
      return E_NOTIMPL;
...
HRESULT CFileDialogW::AddSeparator(DWORD dwIDCtl)
{
   if (!m_bVistaStyle)
   {
      return E_NOTIMPL;
...
HRESULT CFileDialogW::AddText(DWORD dwIDCtl, const CStringW& strText)
{
   if (!m_bVistaStyle)
   {
      return E_NOTIMPL;
...
HRESULT CFileDialogW::SetControlLabel(DWORD dwIDCtl, const CStringW& strLabel)
{
   if (!m_bVistaStyle)
   {
      return E_NOTIMPL;
...
HRESULT CFileDialogW::GetControlState(DWORD dwIDCtl, CDCONTROLSTATEF& dwState)
{
   if (!m_bVistaStyle)
   {
      return E_NOTIMPL;
...
HRESULT CFileDialogW::SetControlState(DWORD dwIDCtl, CDCONTROLSTATEF dwState)
{
   if (!m_bVistaStyle)
   {
      return E_NOTIMPL;
...
HRESULT CFileDialogW::GetEditBoxText(DWORD dwIDCtl, CStringW& strText)
{
   if (!m_bVistaStyle)
   {
      return E_NOTIMPL;
...
HRESULT CFileDialogW::SetEditBoxText(DWORD dwIDCtl, const CStringW& strText)
{
   if (!m_bVistaStyle)
   {
      return E_NOTIMPL;
...
HRESULT CFileDialogW::GetCheckButtonState(DWORD dwIDCtl, BOOL& bChecked)
{
   if (!m_bVistaStyle)
   {
      return E_NOTIMPL;
...
HRESULT CFileDialogW::SetCheckButtonState(DWORD dwIDCtl, BOOL bChecked)
{
   if (!m_bVistaStyle)
   {
      return E_NOTIMPL;
...
HRESULT CFileDialogW::AddControlItem(DWORD dwIDCtl, DWORD dwIDItem, const CStringW& strLabel)
{
   if (!m_bVistaStyle)
   {
      return E_NOTIMPL;
...
HRESULT CFileDialogW::RemoveControlItem(DWORD dwIDCtl, DWORD dwIDItem)
{
   if (!m_bVistaStyle)
   {
      return E_NOTIMPL;
...
HRESULT CFileDialogW::GetControlItemState(DWORD dwIDCtl, DWORD dwIDItem, CDCONTROLSTATEF& dwState)
{
   if (!m_bVistaStyle)
   {
      return E_NOTIMPL;
...
HRESULT CFileDialogW::SetControlItemState(DWORD dwIDCtl, DWORD dwIDItem, CDCONTROLSTATEF dwState)
{
   if (!m_bVistaStyle)
   {
      return E_NOTIMPL;
...
HRESULT CFileDialogW::GetSelectedControlItem(DWORD dwIDCtl, DWORD& dwIDItem)
{
   if (!m_bVistaStyle)
   {
      return E_NOTIMPL;
...
HRESULT CFileDialogW::SetSelectedControlItem(DWORD dwIDCtl, DWORD dwIDItem)
{
   if (!m_bVistaStyle)
   {
      return E_NOTIMPL;
...
HRESULT CFileDialogW::StartVisualGroup(DWORD dwIDCtl, const CStringW& strLabel)
{
   if (!m_bVistaStyle)
   {
      return E_NOTIMPL;
...
HRESULT CFileDialogW::EndVisualGroup()
{
   if (!m_bVistaStyle)
   {
      return E_NOTIMPL;
...
HRESULT CFileDialogW::MakeProminent(DWORD dwIDCtl)
{
   if (!m_bVistaStyle)
   {
      return E_NOTIMPL;
...
HRESULT CFileDialogW::SetControlItemText(DWORD dwIDCtl, DWORD dwIDItem, const CStringW& strLabel)
{
   if (!m_bVistaStyle)
   {
      return E_NOTIMPL;
...
void CFileDialogW::OnItemSelected(DWORD, DWORD)
{
   ASSERT_VALID(this);
...
void CFileDialogW::OnButtonClicked(DWORD)
{
   ASSERT_VALID(this);
...
void CFileDialogW::OnCheckButtonToggled(DWORD, BOOL)
{
   ASSERT_VALID(this);
...
void CFileDialogW::OnControlActivating(DWORD)
{
   ASSERT_VALID(this);
...
BOOL CFileDialogW::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT* pResult)
{
   ASSERT(pResult != NULL);
...
////////////////////////////////////////////////////////////////////////////
// CFileDialogW diagnostics

#ifdef _DEBUG
void CFileDialogW::Dump(CDumpContext& dc) const
{
   CDialog::Dump(dc);
   ...
   dc << "\nm_ofn.lpstrFilter = ";
   LPCWSTR lpstrItem = m_ofn.lpstrFilter;
   LPWSTR lpszBreak = L"|";

   while (lpstrItem != NULL && *lpstrItem != '\0')
   {
      dc << lpstrItem << lpszBreak;
      lpstrItem += wcslen(lpstrItem) + 1;
   }
   if (lpstrItem != NULL)
      dc << lpszBreak;
   ...
   if (m_ofn.lpfnHook == (COMMDLGPROC)_AfxCommDlgProc2)
...
IMPLEMENT_DYNAMIC(CFileDialogW, CCommonDialog)

//////////////////////////////////////////////////////////////////////////////
//// Folder Picker common dialog helper
//
//CFolderPickerDialog::CFolderPickerDialog(LPCWSTR lpszFolder, DWORD dwFlags, CWnd* pParentWnd, DWORD dwSize, BOOL fNonFileSystemFolders) :
//   CFileDialogW(TRUE, NULL, lpszFolder, dwFlags, NULL, pParentWnd, dwSize, TRUE)
//{
...
//}
//
//IMPLEMENT_DYNAMIC(CFolderPickerDialog, CFileDialogW)
//
//////////////////////////////////////////////////////////////////////////////
//
...
UINT_PTR CALLBACK _AfxCommDlgProc2(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
   ...
   {
      ASSERT_KINDOF(CFileDialogW, pThreadState->m_pAlternateWndInit);
   ...
   {
      _afxMsgLBSELCHANGE2 = ::RegisterWindowMessage(LBSELCHSTRING);
      _afxMsgSHAREVI2 = ::RegisterWindowMessage(SHAREVISTRING);
      _afxMsgFILEOK2 = ::RegisterWindowMessage(FILEOKSTRING);
      _afxMsgCOLOROK2 = ::RegisterWindowMessage(COLOROKSTRING);
      _afxMsgHELP2 = ::RegisterWindowMessage(HELPMSGSTRING);
      _afxMsgSETRGB2 = ::RegisterWindowMessage(SETRGBSTRING);
      return (UINT)AfxDlgProc(hWnd, message, wParam, lParam);
   }

   if (message == _afxMsgHELP2 ||
   ...
   if (pDlg->IsKindOf(RUNTIME_CLASS(CFileDialogW)))
   {
      // If we're exploring then we are not interested in the Registered messages
      if (((CFileDialogW*)pDlg)->m_ofn.Flags & OFN_EXPLORER)
         return 0;
   }
   ...
   if (message == _afxMsgSHAREVI2)
   {
      ASSERT_KINDOF(CFileDialogW, pDlg);
      return ((CFileDialogW*)pDlg)->OnShareViolation((LPCWSTR)lParam);
   }
   else if (message == _afxMsgFILEOK2)
   {
      ASSERT_KINDOF(CFileDialogW, pDlg);

      ((CFileDialogW*)pDlg)->m_pofnTemp = (OPENFILENAMEW*)lParam;
      BOOL bResult = ((CFileDialogW*)pDlg)->OnFileNameOK();
      ((CFileDialogW*)pDlg)->m_pofnTemp = NULL;

      return bResult;
   }
   else if (message == _afxMsgLBSELCHANGE2)
   {
      ASSERT_KINDOF(CFileDialogW, pDlg);
      ((CFileDialogW*)pDlg)->OnLBSelChangedNotify((UINT)wParam, LOWORD(lParam),
         HIWORD(lParam));
      return 0;
   }
   //else if (message == _afxMsgCOLOROK2)
   //{
   //   ASSERT_KINDOF(CColorDialog, pDlg);
   //   return ((CColorDialog*)pDlg)->OnColorOK();
   //}
   else if (message == _afxMsgSETRGB2)
   {
