...
TemplateLeftWnd::TemplateLeftWnd()
{
   ...
   m_pbitmap = LoadPNG(MAKEINTRESOURCEA(IDB_PNG_COMPASS_BORDERED), "PNG", NULL);
...
void TemplateLeftWnd::OnDraw(CDC * pdc, Gdiplus::Graphics * pgraphics)
{
   ...
   int cxDst = m_pbitmap->GetWidth()*size_scaler() * 0.165;
   int cyDst = m_pbitmap->GetHeight()*size_scaler() * 0.165;

   Gdiplus::RectF rectDst(
      (rectClient.Width() - cxDst) / 2,
      24 * size_scaler(),
      cxDst,
      cyDst);
...
void TemplateLeftWnd::OnSize(UINT nType, int cx, int cy)
{
   ...
   int iButtonHeight = 40 * size_scaler();

   int y = 80 * size_scaler();
