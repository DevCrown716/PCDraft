...
void CScrollHelper::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
    ...
    {
        m_scrollPos.cx += deltaPos;
        m_attachWnd->SetScrollPos(SB_HORZ, m_scrollPos.cx, FALSE);
        //m_attachWnd->ScrollWindow(-deltaPos, 0);
        m_attachWnd->Invalidate(FALSE);
    }
}
...
void CScrollHelper::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
    ...
    {
        m_scrollPos.cy += deltaPos;
        //m_attachWnd->LockWindowUpdate();
        m_attachWnd->SetScrollPos(SB_VERT, m_scrollPos.cy, FALSE);
        m_attachWnd->Invalidate(FALSE);
        //m_attachWnd->RedrawWindow(nullptr, nullptr, RDW_UPDATENOW);
        //m_attachWnd->ScrollWindow(0, -deltaPos);
        //m_attachWnd->Invalidate(FALSE);
        //m_attachWnd->UnlockWindowUpdate();
    }
}
                                    ...
                                    LONG& pageSize, LONG& scrollPos, LONG& deltaPos)
{
    ...
    else
    {
        ...
        //m_attachWnd->ShowScrollBar(bar, FALSE);
    }
    ...
    si.nPage  = pageSize;
    si.nPos   = scrollPos;
    m_attachWnd->SetScrollInfo(bar, &si, FALSE);
}
