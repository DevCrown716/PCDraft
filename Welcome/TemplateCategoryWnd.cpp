...
void TemplateCategoryWnd::layout()
{
   ...
   m_iThumbnailSize = 180 * size_scaler();

   int iButtonHeight = m_iThumbnailSize;
   int iButtonWidth = m_iThumbnailSize;

   CRect rectTotal;

   int iColumn = 0;

   m_iMaxColumnCount = (cx / iButtonWidth) - 1;

   if (m_iMaxColumnCount <= 0)
   {
   ...
   int iSpacing = ( cx - m_iMaxColumnCount * iButtonWidth) / (m_iMaxColumnCount + 1);
...
void TemplateCategoryWnd::OnDrawItem(int iItem, CDC * pdc, Gdiplus::Graphics * pgraphics, Item & item)
{
   ...
   rectClient.DeflateRect(20 * size_scaler(), 20 * size_scaler());
...

   {
      ...
      rIcon.Width = m_iThumbnailSize ;
      rIcon.Height = m_iThumbnailSize * 0.90 ;
...
#undef new
            ...
            double iMargin = 10.0 * size_scaler();
            ...
            //r1.X += iMargin;
            //r1.Y += iMargin;
            //r1.Width -= iMargin * 2.0;
            //r1.Height -= iMargin * 2.0;
            g.DrawImage(pbitmap, r1);

            Gdiplus::Pen p1(Gdiplus::Color(255, 192, 192, 192), 1.0f);
            r.Width--;
            r.Height--;
      ...
      if (item.m_pbitmap)
      {
         ...
         rIcon.Width = item.m_pbitmap->GetWidth();
         rIcon.Height = item.m_pbitmap->GetHeight();

      }

      rIcon.Y = rectClient.bottom - rIcon.Height - iLineHeight1 - 20 * size_scaler();
      ...
      if(iItem == m_iHover || iItem == m_iSel)
      {
         ...
         rect1.X -= 10 * size_scaler();
         rect1.Y -= 10 * size_scaler();
         rect1.Width += 20 * size_scaler();
         rect1.Height = rectF1.Y + rectF1.Height + 10 * size_scaler() - rect1.Y;

         if (iItem == m_iSel)
         {

            Gdiplus::SolidBrush bBackground(Gdiplus::Color(255, 245, 212, 202));
         ...
         else
         {
            ...
            Gdiplus::SolidBrush bBackground(Gdiplus::Color(255, 222, 222, 222));
      ...
      if (iItem == m_iSel)
      {
         ...
         Gdiplus::Pen pen(Gdiplus::Color(255, 240, 205, 190), 1.0);
...
void TemplateCategoryWnd::OnMouseMove(UINT nFlags, CPoint point)
{
   ...
   if (iHover != m_iHover)
   {
      ...
      if (iHoverOld >= 0 && iHoverOld < m_itema.GetCount())
      {
         ...
         auto rect = m_itema[iHoverOld].m_rect - m_scrollHelper->GetScrollPos();

         rect.InflateRect(30 * size_scaler(), 30 * size_scaler());
      ...
      if (iHover >= 0 && iHover < m_itema.GetCount())
      {
         ...
         auto rect = m_itema[iHover].m_rect - m_scrollHelper->GetScrollPos();
...
void TemplateCategoryWnd::OnClick(int iItem)
{
   ...
   if (((CPCDraftApp*)AfxGetApp())->m_pTemplateWnd)
   {
      ...
      ((CPCDraftApp*)AfxGetApp())->m_pTemplateWnd->Hide();
   ...
   if (((CPCDraftApp*)AfxGetApp())->m_pGetStartedWnd)
   {
      ...
      ((CPCDraftApp*)AfxGetApp())->m_pGetStartedWnd->Hide();
