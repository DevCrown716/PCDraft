...
void BaseWnd::OnButtonPress(int i)
{
   ...
   else if (i == ID_FILE_OPEN)
   {
      ...
      if (papplication->m_pTemplateWnd)
      {
         ...
         papplication->m_pTemplateWnd->Hide();

      }

      if (papplication->m_pGetStartedWnd)
      {

         papplication->m_pGetStartedWnd->Hide();

      }

      papplication->ShowMainFrame();

      papplication->OnFileOpen();
...
int BaseWnd::OnMouseActivate(CWnd* pDesktopWnd, UINT nHitTest, UINT message)
{
   // TODO: Add your message handler code here and/or call default
