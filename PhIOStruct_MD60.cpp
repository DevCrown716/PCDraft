...
namespace formatMD60
{
   ...
   void serialize::operator()(PhTextParams * ptextParams)
   {
      ...
      operator()(ptextParams->fTextFont);
      operator()(ptextParams->fTextSize);
      operator()(ptextParams->fTextStyle);
      operator()(ptextParams->fReserved1);
      ...
      operator()(ptextParams->fInPixels);
   ...
   void serialize::operator()(PhDimFormat * pdimFormat)
   {
      ...
      operator()(pdimFormat->dfDimLabelHdl);
   ...
   void serialize::operator()(PhActiveEnvirons * pactiveEnvirons)
   {
      ...
      operator()(pactiveEnvirons->fRulerOrigin);
      ...
      operator()(pactiveEnvirons->fHasGrid_OLD);
      ...
      operator()(pactiveEnvirons->fReserved02);
...
namespace formatMD60
{
   ...
   void PhIOContext::ConvertPenModel(const PenModel& pm,PhPenModel& phpm)
   {
      ...
      {
         UINT idx = pm.m_crColor & 0x00ffffff;
         PALETTEENTRY pe;
   ...
   void PhIOContext::ConvertFillModel(const FillModel& fm,PhFillModel& phfm)
   {
      ...
      {
         UINT idx =fm.m_crColor & 0x00ffffff;
         PALETTEENTRY pe;
         pPalette->GetPaletteEntries(idx,1,&pe);
   ...
   void PhIOContext::ConvertTextParams(const TextParams& tp,PhTextParams& phtp)
   {
      ...
      phtp.fTextFont = lFont;
      phtp.fTextSize = lSize;

      switch(tp.m_nSpacing)
      {
      case ::kSingleSpace:
         phtp.fTextSpace=::formatMD60::kSingleSpace;
         break;
      case ::k1_5Space:
         phtp.fTextSpace=::formatMD60::kOne_halfSpace;
         break;
      case ::kDoubleSpace:
         phtp.fTextSpace=::formatMD60::kDoubleSpace;
         break;
      }
      switch(tp.m_nJust)
      {
      case ::kAlignLeft:
         phtp.fHorizTextJust=teJustLeft;
         break;
      case ::kAlignCenter:
         phtp.fHorizTextJust=teJustCenter;
         break;
      case ::kAlignRight:
         phtp.fHorizTextJust=teJustRight;
         break;
      }
      switch(tp.m_nCase)
      {
   ...
   void PhIOContext::ConvertDimFmt(const DimFormat& fmt,PhDimFormat& phfmt)
   {
      ...
      phfmt.dfTextDisplay=(::formatMD60::TextDisplayType) fmt.nTextDisplay;
      phfmt.dfLeader= (::formatMD60::LeaderType) fmt.nLeader;
      phfmt.dfRDSymbol=(::formatMD60::RDSymbolType) fmt.nRDSymbol;
      phfmt.dfTolerance=(::formatMD60::ToleranceType) fmt.nTolerance;
   ...
   void  PhMDShape::operator()(serialize & serialize)
   {
      ...
      serialize(fDatum);
      serialize(fPrev); // fPrev???
      serialize(fOwnerGroup);
      serialize(cRef);
      serialize(fObjectProps);
      serialize(fObjectInfo);
      serialize(fRot);
      serialize(&cPenModel);
      serialize(fShapeFlags);
   ...
   void  PhMDTextScript::operator()(serialize & serialize)
   {
      complex_type_context complextypecontext(&serialize);
      serialize(scrpStartChar);
      serialize(scrpHeight);
      serialize(scrpAscent);
      serialize(scrpFont);
      serialize(scrpFace);
      serialize(theFaceChar);
      serialize(scrpColor);
      serialize(scrpSize);
   }
...
{
   //CString str2;
   //char * pBuf = str2.GetBufferSetLength(str.GetLength() + 1);
   //for(int i=0; i<str.GetLength() ; i++)
   //   pBuf[i]=formatMD60::mac2pc[(BYTE)str[i]];
   //str2.ReleaseBuffer();
   return str;
   //return get_cp(get_uni(str,CP_UTF8),10000);
}
...
CString maccp_to_utf8(CString str)
{
   ...
   ////char * pBuf = str2.GetBufferSetLength(str.GetLength() + 1);
   ////for(int i=0; i<str.GetLength() ; i++)
   ////   pBuf[i]=formatMD60::pc2mac[(BYTE)str[i]];
   ////str2.ReleaseBuffer();
   return str;
   //return get_cp(get_uni(str,10000),CP_UTF8);
}
