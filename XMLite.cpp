...
static const TCHAR szXMLDOCTYPEOpen[] = _T("<!DOCTYPE");
static const TCHAR szXMLDOCTYPEClose[] = _T(">");
...
// cst(ca2) 2018-02-20
//========================================================
LPTSTR _tagXMLNode::LoadDOCTYPE(LPCTSTR pszXml,LPPARSEINFO pi /*= &piDefault*/)
{
   // find the end of comment
   LPTSTR end = _tcsenistr(pszXml,szXMLDOCTYPEClose,sizeof(szXMLDOCTYPEClose) - 1,pi ? pi->escape_value : 0);
   if(end == NULL)
      return NULL;
...
LPXNode _tagXMLNode::get_pinfo_node(CString strName)
{
   ...
   int iFind = IndexByValue("key",strName);

   if(iFind >= 0)
   {

      XNode * pdict = GetChild(iFind + 1);

      if(pdict != NULL && pdict->name == "dict")
      {

         XNode * parray = pdict->GetChild("array");

         if(parray != NULL)
         {

            XNode * pdict2 = parray->GetChild("dict");

            if(pdict2 != NULL)
            {

               int iFind = pdict2->IndexByValue("key",strName);

               if(iFind >= 0)
               {

                  pnode = pdict2->GetChild(iFind + 1);
...
bool _tagXMLNode::get_pinfo_string(CString & strValue, CString strName)
{

   LPXNode pnode = get_pinfo_node(strName);

   if(pnode == NULL || pnode->name != "string")
   {
...
bool _tagXMLNode::get_child_numbera(CArray < double > & numbera)
{
   ...
   for(int i = 0; i < childs.size(); i++)
   {
      ...
      numbera.Add(::atof(childs[i]->value));
...
bool _tagXMLNode::get_pinfo_real(double & dReal, CString strName)
{

   LPXNode pnode = get_pinfo_node(strName);

   if(pnode == NULL || pnode->name != "real")
   {
   ...
   dReal = strtod(pnode->value, NULL);

   return true;

}

bool _tagXMLNode::get_pinfo_integer(int & iInteger,CString strName)
{

   LPXNode pnode = get_pinfo_node(strName);

   if(pnode == NULL || pnode->name != "integer")
   {
...
LPTSTR LoadOtherNodes( LPXNode node, bool* pbRet, LPCTSTR pszXml, LPPARSEINFO pi /*= &piDefault*/ )
{
	...
	*pbRet = false;
	...
	while( xml && do_other_type )
	{
      ...
      // is DOCTYPE Node?
      if(_tcsnicmp(xml,szXMLDOCTYPEOpen,sizeof(szXMLDOCTYPEOpen) - 1) == 0)
      {
         // processing comment parse
         // return pointer is next node of comment
         xml = node->LoadDOCTYPE(xml,pi);
...
int	_tagXMLNode::IndexByValue(LPCTSTR name,LPCTSTR value)
{
   for(int i = 0 ; i < childs.size(); i++)
   {
      LPXNode node = childs[i];
...
XNode * _tagXMLNode::GetChild(LPCTSTR name,LPCTSTR value)
{
   for(int i = 0 ; i < childs.size(); i++)
   {
      LPXNode node = childs[i];
